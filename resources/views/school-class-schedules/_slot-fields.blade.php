...
<div class="form-group">
    <label for="{{ $prefix }}_weekday">{{ __('Dia') }}</label>
    <select id="{{ $prefix }}_weekday" name="weekday" class="form-control @error('weekday') is-invalid @enderror" required>
        @foreach($weekdays as $weekday => $label)
            <option value="{{ $weekday }}" @selected((int) old('weekday', $slot?->weekday ?? 1) === (int) $weekday)>{{ __($label) }}</option>
        @endforeach
    </select>
    @error('weekday')<div class="invalid-feedback">{{ __($message) }}</div>@enderror
</div>

<div class="form-group">
    <label for="{{ $prefix }}_type">{{ __('Tipo') }}</label>
    <select id="{{ $prefix }}_type" name="type" class="form-control @error('type') is-invalid @enderror" data-slot-type>
        @foreach(\App\Models\SchoolClassScheduleSlot::TYPE_LABELS as $type => $label)
            <option value="{{ $type }}" @selected($slotType === $type)>{{ __($label) }}</option>
        @endforeach
    </select>
...
<div class="form-group" data-slot-component>
    <label for="{{ $prefix }}_component">{{ __('Componente curricular') }}</label>
    <select id="{{ $prefix }}_component" name="school_class_component_id" class="form-control @error('school_class_component_id') is-invalid @enderror">
        ...
        @foreach($assignments as $assignment)
            ...
            <option value="{{ $assignment->id }}" @selected($selectedAssignmentId === $assignment->id) @disabled(! $available)>
                ...
                · {{ $used }}/{{ $limit }} {{ __('aula(s)') }}
            </option>
        @endforeach
    </select>
    @error('school_class_component_id')<div class="invalid-feedback">{{ __($message) }}</div>@enderror
</div>

<div class="form-group" data-slot-label hidden>
    <label for="{{ $prefix }}_label">{{ __('Nome do intervalo') }}</label>
    <input id="{{ $prefix }}_label" name="label" class="form-control @error('label') is-invalid @enderror" value="{{ old('label', $slot?->label) }}" placeholder="{{ __('Intervalo, almoço, estudo...') }}">
    @error('label')<div class="invalid-feedback">{{ __($message) }}</div>@enderror
</div>

<div class="row">
    <div class="col-6 form-group">
        <label for="{{ $prefix }}_starts_at">{{ __('Início') }}</label>
    ...
    <div class="col-6 form-group">
        <label for="{{ $prefix }}_ends_at">{{ __('Fim') }}</label>
