...
@section('title', __('screens.enrollments'))
...
@section('content')
    <section class="sge-flow-hero mb-4" aria-labelledby="enrollment-overview-title">
        <div>
            <div class="sge-page-kicker">{{ __('screens.academic_routine') }}</div>
            <h2 id="enrollment-overview-title">{{ __('screens.choose_class') }}</h2>
            <p>{{ __('screens.enrollment_intro') }}</p>
        </div>
        <a class="btn btn-primary" href="{{ route('people.index') }}">
            <i class="fas fa-user-plus mr-1" aria-hidden="true"></i>
            {{ __('screens.find_student') }}
        </a>
    </section>
            ...
            <div class="card-header py-3 d-flex align-items-center justify-content-between flex-wrap">
                <div>
                    ...
                    <p class="small text-gray-600 mb-0">
                        {{ __('screens.years_classes_summary', ['years' => $yearsWithClasses->count(), 'classes' => $yearsWithClasses->flatMap->classes->count()]) }}
                    </p>
                </div>
                <a class="btn btn-sm btn-outline-primary sge-icon-action mt-2 mt-md-0" href="{{ route('schools.academic-years.index', $school) }}" aria-label="{{ __('screens.open_school_years', ['school' => $school->name]) }}" title="{{ __('screens.school_years') }}">
                    ...
                    <article class="sge-enrollment-year">
                        <header class="sge-enrollment-year-header">
                            ...
                            <div class="sge-enrollment-year-stats" aria-label="{{ __('screens.year_summary', ['year' => $year->name]) }}">
                                ...
                                <span>{{ __('screens.active_count', ['count' => $year->classes->sum(fn ($class) => $class->enrollments->where('status', \App\Models\StudentEnrollment::STATUS_ENROLLED)->count())]) }}</span>
                            </div>
                        </header>
                            ...
                            @foreach ($year->classes->sortBy('name') as $class)
                                ...
                                <article class="sge-enrollment-class-card">
                                    <div class="sge-enrollment-class-main">
                                        <h4>{{ $class->name }}</h4>
                                        <p>{{ $class->shift ?: __('screens.shift_undefined') }}</p>
                                        <div class="sge-class-course-chips">
                                            ...
                                            @empty
                                                <span>
                                                    <strong>{{ __('screens.without_curriculum') }}</strong>
                                                    <small>{{ __('screens.regularize_before_enrollment') }}</small>
                                                </span>
                                            @endforelse
                                        </div>
                                    </div>
                                    <div class="sge-enrollment-class-side">
                                        ...
                                        <span>{{ __('screens.active_f') }}</span>
                                        @if ($class->enrollments->count() !== $activeEnrollmentCount)
                                            <small>{{ __('screens.in_history', ['count' => $class->enrollments->count()]) }}</small>
                                        @endif
        ...
        <section class="card shadow">
            <div class="card-body">
                <div class="sge-empty-state">
                    ...
                    <p>{{ __('screens.no_school_enrollment') }}</p>
                </div>
            </div>
        </section>
    @endforelse
