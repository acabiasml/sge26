...
<form method="POST" action="{{ $isEdit ? route('academic-years.classes.update', [$academicYear, $class]) : route('academic-years.classes.store', $academicYear) }}">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h2 class="h6 m-0 font-weight-bold text-primary">{{ __('Dados da turma') }}</h2>
        </div>
        <div class="card-body">
            @error('general')
            ...
            @if ($readyCourses->isEmpty())
                <div class="alert alert-warning">
                    {{ __('Cadastre ao menos uma matriz com componentes curriculares antes de criar turmas.') }}
                </div>
            @endif

            <div class="row">
                <div class="col-md-4 form-group">
                    <label for="name">{{ __('Nome da turma') }}</label>
                    <input id="name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $class->name) }}" placeholder="{{ __('3º Ano A') }}" required>
                    @error('name') <div class="invalid-feedback">{{ __($message) }}</div> @enderror
                </div>
                <div class="col-md-3 form-group">
                    <label for="shift">{{ __('Turno') }}</label>
                    <select id="shift" name="shift" class="form-control @error('shift') is-invalid @enderror">
                        <option value="">{{ __('Não definido') }}</option>
                        @foreach(\App\Models\SchoolClass::SHIFT_LABELS as $value => $label)
                            <option value="{{ $value }}" @selected($selectedShift === $value)>{{ __($label) }}</option>
                        @endforeach
                        @if($selectedShift && !array_key_exists($selectedShift, \App\Models\SchoolClass::SHIFT_LABELS))
                            <option value="{{ $selectedShift }}" selected>{{ $selectedShift }} {{ __('(cadastrado anteriormente)') }}</option>
                        @endif
                    </select>
                    @error('shift') <div class="invalid-feedback">{{ __($message) }}</div> @enderror
                </div>
                <div class="col-md-3 form-group">
                    <label for="stage">{{ __('Etapa de ensino') }}</label>
                    <select id="stage" name="stage" class="form-control @error('stage') is-invalid @enderror" data-class-stage required>
                        <option value="">{{ __('Selecione a etapa') }}</option>
                        <option value="{{ \App\Models\AcademicCourse::STAGE_ELEMENTARY }}" @selected($selectedStage === \App\Models\AcademicCourse::STAGE_ELEMENTARY)>{{ __('Ensino Fundamental') }}</option>
                        <option value="{{ \App\Models\AcademicCourse::STAGE_HIGH_SCHOOL }}" @selected($selectedStage === \App\Models\AcademicCourse::STAGE_HIGH_SCHOOL)>{{ __('Ensino Médio') }}</option>
                    </select>
                    @error('stage') <div class="invalid-feedback">{{ __($message) }}</div> @enderror
                </div>
                <div class="col-md-2 form-group">
                    <div class="font-weight-bold text-gray-900">{{ __('Situação') }}</div>
                    <div class="custom-control custom-checkbox mt-2">
                        ...
                        <label class="custom-control-label" for="active">{{ __('Turma ativa') }}</label>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    <label for="starts_period_id">{{ __('Período inicial') }}</label>
                    ...
                    @error('starts_period_id') <div class="invalid-feedback">{{ __($message) }}</div> @enderror
                </div>
                <div class="col-md-6 form-group">
                    <label for="ends_period_id">{{ __('Período final') }}</label>
                    ...
                    @error('ends_period_id') <div class="invalid-feedback">{{ __($message) }}</div> @enderror
                </div>
            </div>

            @if ($academicYear->periods->isEmpty())
                <div class="alert alert-warning" role="alert">
                    {{ __('Cadastre os períodos avaliativos deste ano letivo antes de criar ou editar turmas.') }}
                </div>
            @endif

            <fieldset class="form-group" aria-describedby="course_ids_help" data-matrix-group="base">
                <legend class="col-form-label pt-0 font-weight-bold">{{ __('Formação Geral Básica') }}</legend>
                            ...
                            <div class="border rounded px-3 py-2 h-100 @if(in_array($course->id, $selectedCourseIds, true)) border-primary bg-light @endif">
                                <div class="custom-control custom-checkbox">
                                    ...
                                    <label class="custom-control-label d-block" for="course_id_{{ $course->id }}">
                                        ...
                                        <span class="d-block text-muted small">{{ __($course->stageLabel()) }} · {{ __($course->modalityLabel()) }}</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                    @empty
                        <div class="col-12"><p class="text-muted mb-2">{{ __('Nenhuma matriz de Formação Geral Básica está pronta para vinculação.') }}</p></div>
                    @endforelse
                    <div class="col-12" data-no-compatible-matrix hidden><p class="text-muted mb-2">{{ __('Nenhuma matriz pronta está disponível para a etapa selecionada.') }}</p></div>
                </div>
            </fieldset>
            ...
            <fieldset class="form-group" aria-describedby="course_ids_help" data-matrix-group="itinerary" @if($selectedStage !== \App\Models\AcademicCourse::STAGE_HIGH_SCHOOL) hidden @endif>
                <legend class="col-form-label pt-0 font-weight-bold">{{ __('Itinerário Formativo') }}</legend>
                <div class="row" role="group">
                    ...
                    @empty
                        <div class="col-12"><p class="text-muted mb-2">{{ __('Nenhuma matriz de Itinerário Formativo está pronta para vinculação.') }}</p></div>
                    @endforelse
                </div>
            </fieldset>
            <small id="course_ids_help" class="form-text text-muted mb-3">{{ __('Selecione a matriz da Formação Geral Básica. Para o Ensino Médio, selecione também os itinerários que compõem a turma, quando houver.') }}</small>
            @error('course_ids') <div class="invalid-feedback d-block mb-3">{{ __($message) }}</div> @enderror
    ...
    <div class="d-flex justify-content-between">
        <a class="btn btn-outline-secondary" href="{{ $isEdit ? route('academic-years.classes.show', [$academicYear, $class]) : route('academic-years.show', $academicYear) }}">{{ __('Cancelar') }}</a>
        <button class="btn btn-primary" type="submit" @disabled($readyCourses->isEmpty() || $academicYear->periods->isEmpty())>{{ $isEdit ? __('Salvar turma') : __('Cadastrar turma') }}</button>
    </div>
</form>
