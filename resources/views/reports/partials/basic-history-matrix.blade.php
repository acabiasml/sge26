    ...
    <tbody>
        ...
        <tr>
            ...
            @if(! $previous || $previous->formation !== $component->formation)
                <td rowspan="{{ $matrixComponents->where('formation', $component->formation)->count() }}" class="center formation-cell"><div class="vertical-formation"><span>{{ $component->formation ?: '-' }}</span></div></td>
            @endif
            @if(! $previous || $previous->formation !== $component->formation || $previous->knowledge_area !== $component->knowledge_area)
                <td rowspan="{{ $matrixComponents->filter(fn ($item) => $item->formation === $component->formation && $item->knowledge_area === $component->knowledge_area)->count() }}" class="area-label">{{ $component->knowledge_area ?: '-' }}</td>
            @endif
            ...
            @foreach($history->years as $year)
                ...
                @else
                    ...
                    <td class="center score-cell">{{ $year->displaysCompletedWorkload() && $record?->workload_hours !== null ? number_format((float) $record->workload_hours, 0, ',', '') : '-' }}</td>
                @endif
            @endforeach
        </tr>
        @endforeach
