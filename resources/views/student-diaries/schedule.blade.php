...
@section('page-actions')
    <a target="_blank" rel="noopener" class="btn btn-sm btn-outline-primary shadow-sm sge-icon-action" href="{{ route('student-diaries.schedule-pdf', $enrollment) }}" aria-label="{{ __('Imprimir meu horário') }}" title="{{ __('Imprimir meu horário') }}">
...
@section('content')
    ...
    <section class="card shadow" aria-labelledby="student-schedule-title">
        <div class="card-header py-3">
            <h2 id="student-schedule-title" class="h6 m-0 font-weight-bold text-primary">{{ __('Horário semanal') }}</h2>
        </div>
        <div class="card-body">
            ...
            @if($slots->isEmpty())
                <p class="mb-0">{{ __('Nenhum horário cadastrado para sua turma.') }}</p>
            @else
                            ...
                            @foreach($daySlots as $slot)
                                ...
                                <div class="col-lg-4 col-md-6 mb-3">
                                    <div class="sge-teacher-schedule-card" style="border-left-color: {{ $colors['border'] }}; background: {{ $colors['background'] }};">
                                        ...
                                        <small>{{ $teacher?->full_name ?? __('Docência não definida') }}</small>
                                    </div>
                                </div>
                            @endforeach
                        </div>
