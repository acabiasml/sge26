...
@section('title', __('Componente - ').$component->name)
@section('page-title', __('Componente: ').$component->name)
...
@section('content')
    <nav class="sge-section-nav sge-academic-tabs mb-4" aria-label="{{ __('Áreas do componente curricular') }}" role="tablist" data-section-tabs data-default-tab="{{ $errors->any() ? 'edicao' : 'resumo' }}">
        <a href="#section-resumo" class="sge-section-nav-item" data-academic-tab="resumo" role="tab"><i class="fas fa-clipboard-list"></i><span>{{ __('Resumo') }}</span><small>{{ __('dados do componente') }}</small></a>
        <a href="#section-edicao" class="sge-section-nav-item" data-academic-tab="edicao" role="tab"><i class="fas fa-pen"></i><span>{{ __('Edição') }}</span><small>{{ __('configuração curricular') }}</small></a>
    </nav>
    ...
    <div class="row">
            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <dl class="mb-0 sge-academic-summary-details">
                        ...
                        <dt>{{ __('Matriz') }}</dt>
                        <dd>{{ $course->name }} · {{ __($course->stageLabel()) }} · {{ __($course->modalityLabel()) }}</dd>
                        <dt>{{ __('Formação') }}</dt>
                        <dd>{{ \App\Support\CurriculumCatalog::formationLabelForArea($course, $component->area) }}</dd>
                        <dt>{{ __('Área') }}</dt>
                        <dd>{{ $component->area?->name ?? __('Não definida') }}</dd>
                        <dt>{{ __('Situação') }}</dt>
                        <dd><span class="badge badge-{{ $component->active ? 'success' : 'secondary' }}">{{ $component->active ? __('Ativo') : __('Inativo') }}</span></dd>
                        <dt>{{ __('Duração') }}</dt>
                        <dd>{{ $component->startsPeriod?->name ?? __('início da turma') }} {{ __('até') }} {{ $component->endsPeriod?->name ?? __('fim da turma') }}</dd>
        ...
        <div id="section-edicao" class="col-12 mb-4 sge-anchor-section" data-academic-panel="edicao" role="tabpanel">
            <div class="card shadow">
                <div class="card-header py-3">
                    <h2 class="h6 m-0 font-weight-bold text-primary">{{ __('Gerenciar componente') }}</h2>
                </div>
                <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('academic-years.courses.components.update', [$academicYear, $course, $component]) }}">
                            ...
                            <div class="row">
                                <div class="col-md-6 form-group">
                                    <label for="component_name">{{ __('Componente') }}</label>
                                ...
                                <div class="col-md-6 form-group" data-workload-choice>
                                    ...
                                    <div class="custom-control custom-radio custom-control-inline">
                                        ...
                                        <label class="custom-control-label" for="component_workload_mode_weekly">{{ __('Aulas por semana') }}</label>
                                    </div>
                                    <div class="custom-control custom-radio custom-control-inline">
                                        ...
                                        <label class="custom-control-label" for="component_workload_mode_total">{{ __('Total de horas') }}</label>
                                    </div>
                                    <div class="mt-2" data-workload-field="weekly_lessons">
                                        <label for="component_weekly_lessons">{{ __('Aulas por semana') }}</label>
                                    ...
                                    <div class="mt-2" data-workload-field="workload_hours">
                                        <label for="component_workload_hours">{{ __('Carga horária total') }}</label>
                                        <div class="input-group"><input id="component_workload_hours" name="workload_hours" data-mask="digits" data-mask-max="5" inputmode="numeric" autocomplete="off" class="form-control" value="{{ old('workload_hours', $component->workload_hours) }}"><div class="input-group-append"><span class="input-group-text">{{ __('horas') }}</span></div></div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-md-4 form-group">
                                    <label for="component_ends_period_id">{{ __('Período final') }}</label>
                            ...
                            <div class="form-group">
                                <label for="component_notes">{{ __('Observações') }}</label>
                                <textarea id="component_notes" name="notes" class="form-control" rows="3">{{ old('notes', $component->notes) }}</textarea>
                            </div>
                            <input type="hidden" name="active" value="1">
                            <button class="btn btn-primary" type="submit">{{ __('Salvar componente') }}</button>
                        </form>
                    @else
                        <p class="mb-0 text-gray-600">{{ __('Ano letivo aprovado. O componente está bloqueado para edição estrutural.') }}</p>
                    @endif
                </div>
            </div>
        </div>
